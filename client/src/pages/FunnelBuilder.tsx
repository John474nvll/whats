...
export default function FunnelBuilder() {
  ...
  const createFunnel = useMutation({
      ...
      const res = await apiRequest('POST', '/api/funnels', {
        ...
        steps: [
          ...
          },
          {
            ...
            platform: 'instagram',
            action: 'send_dm',
            config: { message: '¡Hola! Gracias por tu interés.' },
          },
        ],
      });
      return res.json();
    ...
    onSuccess: () => {
      ...
      toast({
        title: 'Embudo creado',
        description: 'El embudo se ha creado correctamente.',
      });
    },
  });
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Crea flujos de trabajo automatizados entre tus redes sociales.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {funnels?.map((funnel) => (
          <Card
            key={funnel.id}
            className="hover-elevate border-border/50 bg-card/50 backdrop-blur-sm"
          >
        ...
        {funnels?.length === 0 && (
          <div className="col-span-full py-12 text-center border-2 border-dashed rounded-xl border-border/50">
            <p className="text-muted-foreground">
              No tienes embudos creados. Comienza creando uno nuevo.
            </p>
          </div>
        )}
      </div>
