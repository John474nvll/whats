...
const mockTasks: Task[] = [
  {
    id: 1,
    title: 'Revisar propuesta de campana',
    description: 'Revisar y aprobar la propuesta de marketing Q1',
    status: 'pending',
    priority: 'high',
    dueDate: '2026-01-25',
  },
  {
    ...
    description: 'Mejorar el copy y CTAs de la pagina principal',
    status: 'in_progress',
    priority: 'medium',
    dueDate: '2026-01-28',
  },
  {
    id: 3,
    title: 'Configurar automatizacion email',
    description: 'Setup de secuencias de bienvenida',
    status: 'completed',
    priority: 'low',
    dueDate: '2026-01-20',
  },
  {
    id: 4,
    title: 'Analizar metricas Instagram',
    description: 'Revisar engagement del ultimo mes',
    status: 'pending',
    priority: 'medium',
    dueDate: '2026-01-30',
  },
  {
    id: 5,
    title: 'Preparar contenido semanal',
    description: 'Crear 7 posts para la proxima semana',
    status: 'in_progress',
    priority: 'high',
    dueDate: '2026-01-26',
  },
...
export default function Tasks() {
  ...
  const createTaskMutation = useMutation({
    mutationFn: async (data: typeof newTask) => {
      const res = await apiRequest('POST', '/api/crm/tasks', {
        ...data,
        status: 'pending',
      });
      return res.json();
    ...
    <div className="p-4 md:p-6 space-y-6 bg-slate-950 min-h-screen">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            Organiza y completa tus tareas eficientemente
          </p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger
            className="w-40 bg-slate-900/40 border-white/10 rounded-xl"
            data-testid="select-task-status-filter"
          >
                        ...
                        <CardContent className="p-4 space-y-3">
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <h4
                                className={`font-bold text-sm ${task.status === 'completed' ? 'line-through text-slate-500' : 'text-white'}`}
                              >
                                {task.title}
                              </h4>
                          ...
                          <div className="flex items-center justify-between text-xs">
                            <div className="flex items-center gap-2">
                              <Badge
                                className={`${priorityConfig[task.priority].color} bg-white/5 text-[9px] uppercase`}
                              >
