...
export default function Funnels() {
  ...
  const createFunnel = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: 'Funnel creado',
        description: 'Tu nuevo embudo de ventas está listo',
      });
      setCreateOpen(false);
        ...
        {isLoading ? (
          ...
          <Card className="dark:bg-slate-900/40 dark:border-white/5">
            <CardContent className="py-16 text-center">
              ...
              <p className="text-muted-foreground mb-6">
                Crea tu primer embudo de ventas para empezar a convertir
              </p>
              <Button
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredFunnels.map((funnel) => (
              <Card
                key={funnel.id}
                className="dark:bg-slate-900/40 dark:border-white/5 hover:dark:border-primary/30 transition-all group"
              >
                  ...
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      ...
                      <CardDescription className="line-clamp-1">
                        {funnel.description || 'Sin descripción'}
                      </CardDescription>
                    </div>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex gap-2 pt-2">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        const newStatus =
                          funnel.status === 'active' ? 'paused' : 'active';
      ...
      <DialogContent className="max-w-4xl dark:bg-slate-900 dark:border-white/10">
        ...
        <Tabs defaultValue="overview" className="mt-4">
          <TabsList className="dark:bg-slate-800 p-1 rounded-xl">
            ...
            <TabsTrigger value="stages" className="rounded-lg font-bold">
              Etapas
            </TabsTrigger>
            <TabsTrigger value="analytics" className="rounded-lg font-bold">
              Analíticas
            </TabsTrigger>
          </TabsList>
              ...
              <Card className="dark:bg-slate-800/50 dark:border-white/5">
                <CardContent className="p-4 text-center">
                  ...
                  <p className="text-2xl font-black dark:text-white">
                    {funnel.totalVisitors
                      ? (
                          (funnel.totalConversions / funnel.totalVisitors) *
                          100
                        ).toFixed(1)
                      : 0}
                    %
            ...
            {funnel.stages && funnel.stages.length > 0 ? (
              <div className="space-y-3">
                {funnel.stages.map((stage, index) => {
                  const stageConfig = stageTypes.find(
                    (s) => s.value === stage.type,
                  );
