      ...
      <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-6 md:space-y-8">
          ...
          <Tabs defaultValue="platforms" className="space-y-6">
            <TabsList className="grid w-full grid-cols-3 bg-slate-900/50 p-1 rounded-2xl border border-white/5">
              <TabsTrigger value="platforms" className="rounded-xl data-[state=active]:bg-primary data-[state=active]:text-black font-bold">Canales</TabsTrigger>
              <TabsTrigger value="ai" className="rounded-xl data-[state=active]:bg-primary data-[state=active]:text-black font-bold">IA</TabsTrigger>
              <TabsTrigger value="advanced" className="rounded-xl data-[state=active]:bg-primary data-[state=active]:text-black font-bold">API</TabsTrigger>
            </TabsList>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-amber-200/60 leading-relaxed">
                      Para Instagram/Facebook usa Meta Developers. Para WhatsApp Business Platform obtén tokens permanentes. Asegúrate de que los tokens tengan permisos de lectura/escritura.
                    </p>
                  </div>
                </div>
              </div>
                    ...
                    >
                      <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                          <div className={`w-10 h-10 md:w-12 md:h-12 rounded-2xl flex items-center justify-center border border-white/10 ${
                            platform === 'whatsapp' ? 'bg-emerald-500/20' : 
                            platform === 'instagram' ? 'bg-pink-500/20' : 
                            platform === 'facebook' ? 'bg-blue-500/20' : 'bg-slate-800/20'
                          }`}>
                          ...
                          <div>
                            ...
                            <CardDescription className="text-slate-500 text-xs md:text-sm">API Integration v3</CardDescription>
                          </div>
                        </div>
                        ...
                        >
                          <CardContent className="space-y-6 pt-0 p-4 md:p-6 border-t border-white/5 bg-slate-950/30">
                            <div className="sm:hidden flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5">
                              <Label htmlFor={`active-mobile-${platform}`} className="text-xs font-bold text-slate-500 uppercase tracking-widest">Estado del Canal</Label>
                              <Switch 
                            ...
                            <div className="grid gap-2">
                              <Label htmlFor={`access-${platform}`} className="text-xs font-black text-slate-500 uppercase tracking-[0.15em]">Access Token / API Key</Label>
                              <div className="relative">
                                ...
                                <div className="absolute right-3 top-1/2 -translate-y-1/2">
                                  <Button variant="ghost" size="sm" className="h-7 text-[10px] font-black uppercase text-primary hover:bg-primary/10">Sincronizar</Button>
                                </div>
                              </div>
                            </div>
                            ...
                            {platform === 'whatsapp' && (
                              <div className="grid gap-2">
                                <Label htmlFor={`phone-${platform}`} className="text-xs font-black text-slate-500 uppercase tracking-[0.15em]">Phone Number ID</Label>
                                <Input 
                                  id={`phone-${platform}`} 
                            ...
                            <Button className="w-full h-11 md:h-12 rounded-xl bg-primary text-black font-black hover:scale-[1.01] transition-all shadow-lg shadow-primary/10" onClick={() => toast({ title: "Configuración Actualizada" })}>
