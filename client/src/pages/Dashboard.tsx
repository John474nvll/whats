...
import { BarChart3, Users, MessageSquare, Zap, Plus, RefreshCw, Activity, ShoppingCart, Filter, ShieldCheck } from "lucide-react";
...
export default function Dashboard() {
  ...
  const stats = [
    ...
    { title: "Mensajes IA", value: "1.2k", icon: MessageSquare, color: "text-blue-500" },
    { title: "Conversión", value: "68%", icon: BarChart3, color: "text-cyan-neon" },
    { title: "Automatización", value: "85%", icon: Zap, color: "text-yellow-500" },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black tracking-tighter text-white">Escritorio <span className="text-kiwi">v12.0</span></h1>
          <p className="text-slate-400 text-sm font-medium">Panel de control omnicanal Softgan</p>
        </div>
        <div className="flex items-center gap-2">
          <Button onClick={handleSync} variant="outline" className="rounded-full border-white/10 hover:bg-white/5 gap-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map((stat, i) => (
          <Card key={i} className="bg-slate-900/50 border-white/5 backdrop-blur-xl hover-elevate">
      ...
      <div className="grid grid-cols-12 gap-6">
        <Card className="col-span-12 lg:col-span-8 bg-slate-900/50 border-white/5 backdrop-blur-xl">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Badge variant="outline" className="border-kiwi/30 text-kiwi">En Vivo</Badge>
          </CardHeader>
          <CardContent>
        ...
        <Card className="col-span-12 lg:col-span-4 bg-slate-900/50 border-white/5 backdrop-blur-xl">
          <CardHeader>
            <CardTitle className="text-lg font-bold text-white">Estado de Sincronización</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {syncStatus.length > 0 ? (
              syncStatus.map((log: any) => (
                <div key={log.id} className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5">
                  <div className="flex items-center gap-3">
                    <div className={`w-2 h-2 rounded-full ${log.status === 'success' ? 'bg-kiwi' : 'bg-red-500'}`} />
                    <div>
              ...
              <div className="text-center py-8">
                <p className="text-slate-500 text-sm">Sin registros recientes</p>
              </div>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="p-3 rounded-xl bg-kiwi/10 border border-kiwi/20">
                <p className="text-xl font-black text-kiwi">{purchaseOrders.length}</p>
                <p className="text-[10px] text-slate-400">Totales</p>
              </div>
            </div>
            <Link href="/orders">
              <Button variant="outline" className="w-full rounded-xl border-white/10">Ver Órdenes</Button>
            </Link>
          </CardContent>
        </Card>
        ...
        <Card className="col-span-12 md:col-span-6 lg:col-span-4 bg-slate-900/50 border-white/5 backdrop-blur-xl">
          ...
          <CardContent className="space-y-4">
            ...
            <Link href="/funnels">
              <Button variant="outline" className="w-full rounded-xl border-white/10">Gestionar Funnels</Button>
            </Link>
          </CardContent>
        </Card>
