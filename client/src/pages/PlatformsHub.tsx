...
export default function PlatformsHub() {
  ...
  const connectMutation = useMutation({
    mutationFn: async ({
      ...
      if (token.length > 50) {
        toast({
          title: 'Validando SDK Meta...',
          description: 'Conectando con servidores de Facebook/Instagram',
        });
      }
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: 'Plataforma sincronizada con SoftganHub',
      });
      setTokens({});
    },
    onError: (error) => {
  ...
  const verifyPhoneCodeMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        title: 'Teléfono conectado',
        description: 'Tu número de WhatsApp está verificado y listo',
      });
      setPhoneNumber('');
      setVerificationCode('');
      ...
      <div className="max-w-6xl mx-auto space-y-12 relative z-10">
        <header className="space-y-4">
          ...
          <p className="text-xl text-slate-400 font-medium tracking-tight max-w-2xl">
            Gestiona tus canales de comunicación de Meta en una sola interfaz
            neuronal.
          </p>
        </header>
          ...
          <div className="lg:col-span-2 space-y-8">
              ...
              <div className="flex items-center justify-between mb-8">
                <div className="space-y-1">
                  ...
                  <p className="text-slate-400 font-medium">
                    Administra tus perfiles vinculados y sus tokens.
                  </p>
                </div>
                    ...
                    >
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3">
                          <div
                            className={`p-2 rounded-xl ${platformColors[account.platform]?.bg || 'bg-white/5'} ${platformColors[account.platform]?.text || 'text-white'}`}
                          >
                            {(() => {
                              const PlatformIcon =
                          ...
                          <div>
                            ...
                            <Badge
                              variant="outline"
                              className="text-[8px] uppercase tracking-widest border-white/5 text-slate-500 mt-1"
                            >
                              {account.platform}
                            </Badge>
                          </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-black text-kiwi uppercase tracking-widest">
                            Activo
                          </span>
                        </div>
                      </div>
                      <div className="space-y-3">
                        ...
                        <div className="flex gap-2">
                          <Button
                            variant="outline"
                            className="flex-1 h-10 rounded-xl border-white/5 bg-white/5 text-slate-400 text-[10px] font-black uppercase tracking-widest hover:text-white"
                          >
                            Configurar
                          </Button>
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-10 w-10 rounded-xl hover:bg-red-500/10 hover:text-red-500 text-slate-500"
                          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {['whatsapp', 'instagram', 'facebook'].map((platform) => {
                ...
                return (
                  <Card
                    key={platform}
                    className={`group relative overflow-hidden bg-slate-900/40 border-white/5 backdrop-blur-3xl rounded-[2rem] transition-all duration-500 hover:scale-[1.02] ${isConnected ? 'border-kiwi/20 shadow-[0_20px_40px_rgba(34,197,94,0.1)]' : 'hover:border-white/10'}`}
                  >
                    <div
                      className={`absolute inset-0 bg-gradient-to-br from-${platform === 'whatsapp' ? 'green-500' : platform === 'instagram' ? 'pink-500' : 'blue-500'}/10 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
                    />
                    <CardHeader className="relative z-10 p-6">
                      <div className="flex justify-between items-start mb-4">
                        <div
                          className={`p-3 rounded-2xl ${colors.bg} ${colors.text} shadow-xl`}
                        >
                        ...
                        {isConnected ? (
                          ...
                          <Badge
                            variant="outline"
                            className="border-white/10 text-slate-500 text-[10px] uppercase tracking-widest px-3 py-1 rounded-full"
                          >
                            Off
                          </Badge>
                        )}
                      </div>
                      <CardTitle className="text-xl font-black capitalize text-white">
                        {platform}
                      </CardTitle>
                    ...
                    <CardContent className="relative z-10 p-6 pt-0 space-y-4">
                      {!isConnected && (
                        <div className="space-y-4">
                          ...
                          <Button
                              ...
                              if (platform === 'whatsapp') {
                                setTimeout(() => {
                                  ...
                                  toast({
                                    ...
                                    description: 'Número vinculado con éxito',
                                  });
                                  setConnecting(null);
                                }, 1500);
                              } else {
                            ...
                            disabled={
                              !tokens[platform] || connecting === platform
                            }
                          >
                            {connecting === platform ? (
                      ...
                      {isConnected && (
                        <Button
                          variant="outline"
                          className="w-full border-white/5 bg-white/5 text-slate-400 font-bold h-11 rounded-xl hover:bg-white/10 hover:text-white transition-all text-xs uppercase tracking-widest"
                        >
                          Configurar
                        </Button>
                      )}
            ...
            <Card className="rounded-[3rem] bg-slate-900/40 border-white/5 backdrop-blur-3xl p-4 overflow-hidden shadow-2xl">
              <CardHeader className="p-8">
                ...
                <CardDescription className="text-lg text-slate-400 font-medium tracking-tight">
                  Envía comunicaciones directas a través de tus canales
                  vinculados.
                </CardDescription>
              </CardHeader>
              <CardContent className="p-8 pt-0 space-y-10">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-4">
                    <label className="text-xs font-black uppercase tracking-[0.25em] text-kiwi ml-2">
                      1. Seleccionar Canal
                    </label>
                    <div className="flex gap-2">
                  ...
                  <div className="space-y-4">
                    <label className="text-xs font-black uppercase tracking-[0.25em] text-cyan-neon ml-2">
                      2. Destinatario
                    </label>
                    <Input
                ...
                <div className="space-y-4">
                  <label className="text-xs font-black uppercase tracking-[0.25em] text-slate-500 ml-2">
                    3. Componer Mensaje
                  </label>
                  <Textarea
                ...
                >
                  {sendMessageMutation.isPending ? (
                    ...
                    <>
                      ENVIAR MENSAJE NEURONAL <Send className="ml-4 h-7 w-7" />
                    </>
                  )}
                </Button>
              </CardContent>
          ...
          <aside className="space-y-8">
            ...
            <Card className="rounded-[2.5rem] bg-green-500/10 border-green-500/20 backdrop-blur-3xl overflow-hidden group">
              ...
              <CardHeader className="relative z-10 p-6">
                ...
                <CardDescription className="text-green-300 font-bold">
                  Número Colombiano (+57)
                </CardDescription>
              </CardHeader>
              <CardContent className="relative z-10 p-6 pt-0 space-y-4">
                <div className="space-y-3">
                  {!verifyingPhone ? (
                    <>
                      ...
                      <Button
                        ...
                        disabled={
                          !phoneNumber || sendPhoneCodeMutation.isPending
                        }
                      >
                    ...
                    <>
                      ...
                      <Button
                        ...
                        disabled={
                          !verificationCode || verifyPhoneCodeMutation.isPending
                        }
                      >
                  ...
                  <div className="pt-4 border-t border-white/10">
                    ...
                    <div className="space-y-2">
                      {phoneAccounts.map((account: any) => (
                        <div
                          key={account.id}
                          className="p-3 rounded-lg bg-black/40 border border-green-500/20 flex items-center justify-between"
                        >
                          <span className="text-xs text-slate-300 font-bold">
                            {account.phoneNumber}
                          </span>
                          <Badge className="bg-green-500 text-black text-[8px] font-black">
                            Activo
                          </Badge>
                        </div>
                      ))}
                ...
                <div className="space-y-4">
                  <div className="p-4 rounded-2xl bg-black/40 border border-white/5 hover:border-white/10 transition-all">
                    <p className="text-xs font-black text-pink-500 uppercase tracking-widest mb-2">
                      • Instagram/Facebook: Meta Developers Console
                    </p>
                    <p className="text-xs text-slate-400 leading-relaxed">
                      Accede a developers.facebook.com, crea una app y genera tu
                      token de acceso.
                    </p>
                  </div>
                  <div className="p-4 rounded-2xl bg-black/40 border border-white/5 hover:border-white/10 transition-all">
                    <p className="text-xs font-black text-green-500 uppercase tracking-widest mb-2">
                      • WhatsApp: WhatsApp Business API
                    </p>
                    <p className="text-xs text-slate-400 leading-relaxed">
                      Usa la API oficial de WhatsApp Business para obtener tu
                      token de acceso.
                    </p>
                  </div>
                  <div className="p-4 rounded-2xl bg-black/40 border border-white/5 hover:border-white/10 transition-all">
                    <p className="text-xs font-black text-blue-500 uppercase tracking-widest mb-2">
                      • Asegurate de que los tokens tengan permisos
                    </p>
                    <p className="text-xs text-slate-400 leading-relaxed">
                      Verifica que tus tokens incluyan los permisos necesarios
              ...
              <CardContent className="px-0 space-y-4">
                <div className="space-y-2">
                  <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-slate-500">
                    <span>Cobertura</span>
                    <span>
                      {Math.round((connectedPlatforms.length / 3) * 100)}%
                    </span>
                  </div>
                  <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      initial={{ width: 0 }}
                      animate={{
                        width: `${(connectedPlatforms.length / 3) * 100}%`,
                      }}
                ...
                <div className="pt-4 border-t border-white/5">
                  <p className="text-[10px] text-slate-500 leading-relaxed">
                    Tu terminal neuronal está lista para orquestar campañas
                    multi-canal una vez que vincules tus plataformas
                    principales.
                  </p>
                </div>
              </CardContent>
