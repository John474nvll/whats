...
export default function PurchaseOrders() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Badge
        className={`${styles[status] || styles.pendiente} font-bold uppercase text-[10px] tracking-wider`}
      >
        ...
        <Tabs defaultValue="grid" className="space-y-4">
            ...
            {isLoading ? (
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredOrders.map((order) => (
                  <Card
                    key={order.id}
                    className="dark:bg-slate-900/40 dark:border-white/5 hover:dark:border-primary/30 transition-all"
                  >
          ...
          <TabsContent value="list" className="space-y-2">
            {filteredOrders.map((order) => (
              <Card
                key={order.id}
                className="dark:bg-slate-900/40 dark:border-white/5"
              >
        ...
        <div className="space-y-6">
          <div className="grid md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <p className="font-bold dark:text-white">
                {order.customer?.name || 'No asignado'}
              </p>
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              {((order.items as OrderItem[]) || []).map((item, i) => (
                <div
                  key={i}
                  className="flex justify-between items-center p-3 rounded-lg dark:bg-slate-800/50"
                >
                  <div>
