...
import type { SocialAccount, ChannelConfig, Campaign, SalesFunnel, Product, CustomLink } from "@shared/schema";
      ...
      <div className="space-y-6">
          ...
          <Card className="bg-slate-900/40 border-white/5 rounded-[2rem] overflow-hidden">
            ...
            <CardContent className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="text-kiwi font-bold">{platformCampaigns.length}</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-400">Embudos</span>
                <span className="text-cyan-neon font-bold">{platformFunnels.length}</span>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <Tabs defaultValue="modules" className="space-y-6">
          <TabsList className="bg-slate-900/60 p-1 rounded-2xl border border-white/5">
            <TabsTrigger value="modules" className="rounded-xl data-[state=active]:bg-kiwi data-[state=active]:text-black font-bold text-xs uppercase">Módulos</TabsTrigger>
            <TabsTrigger value="content" className="rounded-xl data-[state=active]:bg-kiwi data-[state=active]:text-black font-bold text-xs uppercase">Contenido</TabsTrigger>
            <TabsTrigger value="automation" className="rounded-xl data-[state=active]:bg-kiwi data-[state=active]:text-black font-bold text-xs uppercase">Automatización</TabsTrigger>
          </TabsList>

          <TabsContent value="modules" className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="bg-slate-900/40 border-white/5 rounded-[2rem] overflow-hidden">
              <CardHeader>
              ...
              <CardContent className="space-y-4">
                {platformCampaigns.length > 0 ? platformCampaigns.slice(0, 3).map(c => (
                  <div key={c.id} className="p-3 rounded-xl bg-white/5 flex justify-between items-center">
                    <span className="text-xs font-bold truncate pr-4">{c.name}</span>
                    <Badge className="bg-kiwi/20 text-kiwi text-[9px] uppercase">{c.status}</Badge>
                  </div>
                )) : (
                  <p className="text-xs text-slate-500 italic">No hay campañas para esta plataforma.</p>
                )}
                <Button variant="outline" className="w-full rounded-xl border-white/10 hover:bg-kiwi/10 hover:border-kiwi/20 text-xs font-bold" asChild>
            ...
            <Card className="bg-slate-900/40 border-white/5 rounded-[2rem] overflow-hidden">
              <CardHeader>
                <CardTitle className="text-lg font-black flex items-center gap-2">
                  <GitFork className="w-5 h-5 text-cyan-neon" /> Embudos (Funnels)
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <Button variant="outline" className="w-full rounded-xl border-white/10 hover:bg-cyan-neon/10 hover:border-cyan-neon/20 text-xs font-bold" asChild>
          ...
          <TabsContent value="content" className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="bg-slate-900/40 border-white/5 rounded-[2rem] overflow-hidden">
              <CardHeader>
                <CardTitle className="text-lg font-black flex items-center gap-2">
                  <ShoppingBag className="w-5 h-5 text-piña" /> Productos Destacados
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                {products.length > 0 ? products.slice(0, 3).map(p => (
                  <div key={p.id} className="p-3 rounded-xl bg-white/5 flex justify-between items-center">
                ...
                <Button variant="outline" className="w-full rounded-xl border-white/10 hover:bg-piña/10 hover:border-piña/20 text-xs font-bold" asChild>
            ...
            <Card className="bg-slate-900/40 border-white/5 rounded-[2rem] overflow-hidden">
              ...
              <CardContent className="space-y-4">
                {platformLinks.length > 0 ? platformLinks.slice(0, 3).map(l => (
                  ...
                  <p className="text-xs text-slate-500 italic">No hay links para esta red.</p>
                )}
                <Button variant="outline" className="w-full rounded-xl border-white/10 hover:bg-cyan-neon/10 hover:border-cyan-neon/20 text-xs font-bold" asChild>
