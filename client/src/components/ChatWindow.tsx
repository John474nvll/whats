...
import { Send, Smile, Paperclip, Bot, Sparkles, AlertCircle } from "lucide-react";
import { format } from "date-fns";
import { useMessages, useSendMessage, useAiAnalysis } from "@/hooks/use-conversations";
          ...
          <div className="relative">
            {conversation.contact.profilePic ? (
              ...
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary/20 to-primary/10 flex items-center justify-center text-primary font-bold">
                {conversation.contact.name?.substring(0, 2).toUpperCase() || "U"}
              </div>
            )}
            <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white rounded-full"></span>
          </div>
          <div>
            <h3 className="font-bold text-foreground">{conversation.contact.name || conversation.contact.platformId}</h3>
            <p className="text-xs text-muted-foreground flex items-center gap-1">
              via <span className="capitalize font-medium">{conversation.channel}</span>
            </p>
          </div>
        </div>
                    ...
                    <div className={cn(
                      ...
                      {msg.sentiment && (
                        <span className={cn(
                          ...
                          msg.sentiment === 'positive' && "text-green-600 bg-green-100",
                          msg.sentiment === 'negative' && "text-red-600 bg-red-100"
                        )}>
                          {msg.sentiment}
                        </span>
                      )}
