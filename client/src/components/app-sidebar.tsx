                  ...
                  {group.items.map((item) => {
                    ...
                    return (
                      <SidebarMenuItem key={item.title}>
                        <SidebarMenuButton
                          asChild
                          className="h-auto p-0 hover:bg-transparent no-default-hover-elevate"
                        >
                          <Link href={item.url}>
                            <div
                            ...
                            >
                              <item.icon
                                className={`h-4 md:h-5 w-4 md:w-5 flex-shrink-0 transition-transform duration-300 group-hover/item:scale-110 ${isActive ? 'text-black' : 'group-hover/item:text-primary'}`}
                              />
                              <span className="text-xs md:text-sm tracking-tight group-data-[state=collapsed]/sidebar:hidden">
                                {item.title}
                              </span>
                              {isActive && (
                                <motion.div
      ...
      <SidebarFooter className="border-t border-white/5 px-2 md:px-4 py-4 space-y-3 bg-gradient-to-t from-black/20 to-transparent group-data-[state=collapsed]/sidebar:flex-row">
          ...
          {[
            {
              ...
              link: 'https://wa.me/3197368698',
            },
            {
              name: 'Instagram',
              icon: Instagram,
              color: 'text-pink-400',
              link: 'https://www.instagram.com/johnncloudy',
            },
            {
              name: 'Facebook',
              icon: Facebook,
        ...
        <div className="text-[9px] text-slate-500 flex justify-between items-center group-data-[state=collapsed]/sidebar:hidden px-1">
          <div className="space-y-0.5">
            <p className="font-bold text-slate-400">SoftganHub</p>
            <p className="opacity-50 tracking-tighter">V9.0 Softgan</p>
          </div>
          <Badge
            variant="outline"
            className="border-primary/40 bg-primary/10 text-primary text-[7px] font-black tracking-widest px-2 py-0.5 rounded-full"
          >
            LIVE
          </Badge>
        </div>
