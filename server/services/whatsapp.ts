...
const WA_ACCESS_TOKEN =
  process.env.WA_ACCESS_TOKEN || process.env.META_ACCESS_TOKEN || '';
const WA_WEBHOOK_VERIFY_TOKEN =
  process.env.WA_WEBHOOK_VERIFY_TOKEN || 'socialhub_verify_token';
...
export function getWhatsAppStatus() {
  return {
    ...
    phoneNumberId: WA_PHONE_NUMBER_ID
      ? '***' + WA_PHONE_NUMBER_ID.slice(-4)
      : null,
    businessAccountId: WA_BUSINESS_ACCOUNT_ID
      ? '***' + WA_BUSINESS_ACCOUNT_ID.slice(-4)
      : null,
  ...
  try {
    ...
    switch (mediaType) {
      case 'image':
        ...
        break;
      case 'video':
        response = await whatsappClient.messages.video(
          { link: mediaUrl, caption },
          recipient,
        );
        break;
      case 'audio':
        response = await whatsappClient.messages.audio(
          { link: mediaUrl },
          recipient,
        );
        break;
      case 'document':
        response = await whatsappClient.messages.document(
          { link: mediaUrl, caption },
          recipient,
        );
        break;
